...
class TestConfigList:
    def test_lists_all_keys_as_defaults(self, temp_home, capsys):
        ...
        for key in (
            ...
            "ui.privacy",
            "poll.budgetShare",
            "sync.broadcastSwitches",
            "sync.broadcastAutoSwitches",
            "sync.followRemoteSwitches",
        ):
            assert key in out
    ...
    def test_json_list(self, temp_home, capsys):
        ...
        assert by_key["sync.broadcastSwitches"]["value"] is True
        assert by_key["sync.followRemoteSwitches"]["value"] is True
        assert by_key["sync.broadcastAutoSwitches"]["value"] is False
